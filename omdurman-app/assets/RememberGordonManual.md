...
*Faithful transcription of the printed rulebook (Phoenix Enterprises, Ltd., 1982), prepared from an OCR pass over `RememberGordonManual.pdf` with character-level errors corrected against the source. Section numbering, wording, and rule content follow the printed rulebook; only obvious OCR artefacts (e.g. "OMDUAIIAN" → "OMDURMAN", "GORDOW" → "GORDON", "namec" → "named") and trivial typography (curly quotes, hyphenation of words broken across lines) have been normalised. No rules have been paraphrased, reordered, or omitted.*
...
Only the small FALL OF KHARTOUM scenario map is used for this game.
