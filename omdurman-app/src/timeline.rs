...
/// Lobby-side entry into review mode: review the in-memory game, or (native)
/// load a finished game from `games/*.jsonl`. Shown only in [`AppState::Lobby`].
pub fn review_entry_ui(
    ...
    egui::Window::new("Review a game")
        ...
        .show(ctx, |ui| {
            ...
            // Load a finished game from disk (native only).
            #[cfg(not(target_arch = "wasm32"))]
            {
                ui.separator();
                ui.label(egui::RichText::new("Saved games").color(egui::Color32::from_gray(190)));
                let games = crate::game_record::list_saved_games();
                if games.is_empty() {
                        ...
                        for (path, name) in &games {
                            if ui.button(name).clicked() {
                                match crate::game_record::load_record_from_jsonl(path) {
                                    Ok(record) => {
                                        timeline.open(record, name.clone());
                                        next_state.set(AppState::Spectating);
                                    }
                                    Err(error) => {
                                        warn!(%error, %path, "failed to load saved game");
                                    }
                                }
                            }
                        }
