...
1. **Build brigade integrity for the +1, then use it.** All four battalions of
   one AE infantry brigade stacked in one hex, all firing at the *same* enemy
   hex, gain +1 on top of the blanket +1 direct-fire accuracy bonus (§5.54,
   §6.24). Keep a brigade together and fire it as a block for a stack-of-4 +
   +1 + +1 attack.
   — §5.54, §6.24

2. **Split stacks only when integrity is irrelevant.** You may divide a stack to
   fire at different hexes (§6.15), but a brigade-integrity stack forfeits the
   +1 if it does not all fire at one hex (§5.54). Use *separate* stacks for
   multi-target coverage; keep the intact brigade on its one hex.
   — §6.15, §5.54

3. **Maxims fire twice — make the second shot count.** Maxims may fire in
   Direct Fire and again in the Maxim Second Fire and Howitzer subphase; they
   may target a hex already fired at in Direct Fire (§6.42). Plan two targets a
   turn for every Maxim battery.
   — §6.42, §6.14

4. **Named gunboats double-dip: direct + howitzer.** The five named British
   gunboats fire their artillery factor as direct fire in Direct Fire and then
   again as howitzer fire in the Maxim/Howitzer subphase (§6.64). Howitzer is
   range 4–10, ignores LOS, and hits the intended hex on an impact roll of 7–10
   (else it scatters — and the result stands even on a friendly hex). A named
   gunboat is a second artillery arm; keep it in that 4–10 window.
   — §6.64, §6.42

5. **No howitzer at night.** Howitzer fire is forbidden on night turns (§6.64,
   §8.1). A named gunboat on a night turn falls back to its direct artillery
   factor only — re-position accordingly.
   — §6.64, §8.1

6. **Halve per unit, floor, min 1.** When range effects halve fire (§6.22),
   halve each unit separately, rounding down but never below 1 (§6.16). Factor
   columns are computed on the halved values.
   — §6.22, §6.16

## Forts, walls, and the approach

7. **Only artillery touches forts and gunboats.** Only artillery may fire at
   forts (eliminate on CRT 2+) and gunboats (sink on CRT 3+) (§6.62, §6.61).
   Your Horse Artillery and battery guns are the only tools for these targets —
...
11. **Constructing the Zariba trades a turn of firepower for a defensive line.**
    ...
    that turn (§5.3). The payoff: −2 to Dervish fire into the hex and no melee
    across the hedge (§9.231), −4 to Dervish fire vs entrenched units and −2
