...
5. ~~**CI runs no tests.**~~ Done: `.github/workflows/ci.yml` now runs, per push/PR — `cargo fmt
   --check`, `cargo clippy -D warnings`, `cargo test --workspace`, the traceability gates, and the
   Kani proof suite (§9), alongside the existing Pages `trunk build --release` deploy.
...
63 proof harnesses verify `SUCCESS` under the [Kani](https://model-checking.github.io/kani/)
model checker: 14 in `omdurman-types/src/lib.rs` (hex geometry), 17 in `omdurman-rules/src/lib.rs`
(`value_enum!` conversions, die arithmetic, victory ladders — 12 written out plus 5 generated by
the `prove_value_enum!` macro, one per `value_enum!` type), 21 in
`omdurman-rules/src/effects.rs` (the `apply_effect` atomicity/monotonicity set), and the
table-backed remainder in `range_effects` (4), `combat_results_table` (2) and
`howitzer_scatter` (1). They live in `#[cfg(kani)] mod verification` blocks inside the file they
verify, so private items stay reachable.
...
- `-Z stubbing` — lets harnesses stub heavy-but-property-neutral cascades (e.g. `end_player_turn`
  under the `AdvancePhase` atomicity harness, `apply_melee_combat` under `ResolveMelee`). A single
...
- **Table contents are not provable.** The four printed tables are authored RON embedded via
  `include_str!` (as `static` consts in `tables_data`), and Kani cannot see through the file
  read. Their shape (including the row lengths that the unchecked `cells[roll - 1]` indexing
  depends on) is asserted in `tables_data::tests::tables_parse_and_have_expected_shape` instead.
- **The effects harnesses prove bounded slices, not the whole engine.** What made them tractable,
  all measured rather than guessed: `GameState`'s per-turn maps and the printed tables moved off
  `HashMap` (hashbrown's probe loop unwound past iteration 600) — and the migration's missed
  container mattered more than any of it: `BoardInfo`'s six `IndexMap`/`IndexSet` fields defaulted
  to `RandomState`, whose `getrandom` seeding is an unmodellable `syscall` on a *live* path
  (`BoardInfo::default()` runs inside every `GameState::new`). It now carries a deterministic
  `BuildHasherDefault<DefaultHasher>` (`board.rs`). Beyond that: unit generation is concrete
  (two units, fixed trip count — a symbolic `take(n)` unrolls the iterator protocol past any
  unwind bound), phases are pinned where a property lives on one arm of the 7-arm phase match
  (CBMC explores paths, so symbolic arms multiply the step count ~10×), and property-neutral
