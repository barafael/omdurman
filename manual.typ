#set page(
  ...
  header: context {
    if counter(page).get().first() > 1 {
      stack(
        dir: ltr,
        spacing: 0.5em,
        text(font: "EB Garamond", size: 9pt, weight: "bold", fill: luma(100), style: "italic", str(counter(page).get().first())),
        text(font: "EB Garamond", size: 9pt, fill: luma(100), style: "italic", "REMEMBER GORDON! — The Battle of Omdurman"),
      )
  ...
  footer: context {
    if counter(page).get().first() > 1 {
      line(length: 100%, stroke: 0.5pt + luma(180))
      v(4pt)
      text(font: "EB Garamond", size: 8pt, fill: luma(120), align(center + bottom, "Copyright © 1982 Phoenix Enterprises, Ltd. — All rights reserved."))
    }
  },
  numbering: "1",
)
...
#align(center, text(font: "EB Garamond", size: 28pt, weight: "bold", "REMEMBER GORDON!"))
#align(center, text(font: "EB Garamond", size: 18pt, weight: "bold", style: "italic", "THE BATTLE OF OMDURMAN"))
#v(6pt)
#align(center, text(font: "EB Garamond", size: 12pt, "A Game of the Anglo-Sudan War, 1898"))
#v(8pt)
#line(length: 60%, stroke: 1pt + luma(60))
#v(8pt)
#align(center, text(font: "EB Garamond", size: 11pt, "Rules of Play"))
#align(center, text(font: "EB Garamond", size: 10pt, "Phoenix Enterprises, Ltd. — 1982"))
#v(12pt)

#align(center, image("Boardgame - Remember_Gordon/Box/box_cover_photo.png", width: 65%))

#pagebreak()

#align(center, text(font: "EB Garamond", size: 14pt, weight: "bold", "CREDITS"))
#v(8pt)

#align(center, text(font: "EB Garamond", size: 10pt, [
...
#align(center, text(font: "EB Garamond", size: 14pt, weight: "bold", "TABLE OF CONTENTS"))
#v(8pt)

#set par(first-line-indent: 0em)

#align(left, text(font: "EB Garamond", size: 11pt, [
  1. Introduction ...................................................... \
  2. Game Components ................................................... \
  3. Getting Started ................................................... \
  4. Turn Sequence .................................................... \
...
#align(center, image("Boardgame - Remember_Gordon/Manual/Elements/Manual_02.jpg", width: 90%))

*Sample Dervish Units:* combat unit (Combat / Melee / Movement values, plus Tribe identifier); Leader (e.g. OSMAN DIGNA); Camel unit (e.g. Danagla, 4-6-12); Fort.

*Sample Anglo-Egyptian Units:* Cavalry (e.g. 21 Lancers); Artillery (e.g. 32 Battery); Old Gunboat (e.g. LORD KITCHENER, 0-0-15); New Gunboat — named (e.g. Sultan, with artillery and howitzer factor, plus movement downstream / movement upstream values); Maxim Guns (fire twice per turn); Infantry (Fire Combat Factor / Melee / Movement, plus Battalion ID and Brigade ID — e.g. "2B" = 2nd British Brigade, "3E" = 3rd Egyptian Brigade).

== Game Parts Inventory
...
#align(center, image("Boardgame - Remember_Gordon/Manual/Elements/CampaignTiming.jpg", width: 80%))
...
*5.11)* The movement allowances of the various unit types are printed directly on the units (see 2.3). A unit may move up to this printed movement allowance, paying varying costs for different terrain types (see the Terrain Effects Chart).

*5.12)* A player may move as many or as few of his units as desired during each movement phase, limited only by the units' movement allowance, the terrain costs paid in moving from hex to hex, and enemy zones of control (see 5.4).

*5.13)* A unit may never accumulate movement points from turn to turn, nor may a unit transfer unused movement points to other units. A unit's unused movement points in any given turn are considered lost.

== Movement Restrictions

*5.21)* In general, naval transport missions are not allowed, i.e. gunboats may not carry any land units. The sole exception is that the Anglo-Egyptian player may transport the surviving units of the "Friendlies" brigade from the east bank of the Nile to the west bank after, and only after, the Dervish east bank unit (Isa Zachneih) has been eliminated. The transport is accomplished in the following sequence:
...
*5.41)* All units except Anglo-Egyptian leaders exert a zone of control (hereafter called a ZOC) into their six adjacent hexes (exception: Gunboats exert a ZOC only against enemy gunboats). Disrupted units have no ZOC.

*5.42)* There is no movement point cost to enter or leave an enemy ZOC.

*5.43)* All units must stop when they enter an enemy ZOC and may move no further that turn. In their next movement phase they may withdraw or, if desired, move directly into another enemy ZOC.

*5.44)* ZOCs do not extend into or out of a Nile River hex (exception: Gunboats, see 5.41). ZOCs do not extend across a khor, into a fort, or into a hex inside the walled city across a wall hexside. ZOCs do extend out of a fort (even if unoccupied), and from a walled city hex into an adjacent non-walled-city hex across a wall hexside. ZOCs also extend out of (but not into) a walled city hex across a gate hexside. ZOCs extend both ways across a breach hexside. ZOCs also extend out of, but not into, a hut or building hex. In the historical scenario ZOCs extend out of, but not into, the Zariba across a Zariba hexside (also in the campaign game if the Zariba is constructed).

== Stacking

*5.51)* No more than four units may occupy a hex, with the exception of leaders and gunboats. All leader units are free stacking, i.e. they may stack in addition to the four-unit-per-hex stacking limitation. Gunboats may not stack with any other unit (Exception: 5.21). Players may move through friendly units at no additional cost in movement points. The stacking limitation applies only at the end of the movement phase and during combat.

*5.52)* The units of different Dervish tribes may not stack together, even if they are the same color (e.g. although both are green, Mulazmin and Jehadia units may not stack with each other).

*5.53)* Leader units are not required to stack. If Dervish leaders elect to stack, however, they may only stack with units of their command (i.e. color). For example, Sheik El Din may only stack with Mulazmins or Jehadias.

*5.54)* _Anglo-Egyptian Brigade Integrity:_ All British, Sudanese, and Egyptian infantry units have their brigade designation printed in the upper right corner (e.g. "2B" = 2nd British Brigade; "3E" = 3rd Egyptian Brigade, etc.). In any combat phase in which all four infantry battalions belonging to any Anglo-Egyptian infantry brigade are stacked in the same hex they are said to have brigade integrity. Stacks having brigade integrity receive a +1 modifier to their fire combat die roll provided they all fire at the same enemy occupied hex. This modifier is in addition to the normal +1 bonus given to all Anglo-Egyptian direct fire attacks (see 6.24).
...
*6.11)* The fire combat factor of the various unit types is printed directly on the units and is a numerical expression of the unit's fire strength.

*6.12)* Fire combat is always voluntary. A unit is never required to fire at enemy units merely because they are in range or adjacent.

*6.13)* If a unit elects to fire, its fire combat factor at an enemy unit, that fire combat factor is unitary. A unit's fire combat factor may not be divided up to fire at enemy units on different hexes.

*6.14)* Players may combine fire during fire combat phase, i.e. they may fire at an enemy-occupied hex with as many friendly units as may legally do so, combining all of their fire combat factors into one attack. Note that in any given fire combat phase, however, a combat unit may only fire once and may only be fired at once (exceptions: Maxim guns and gunboats — see 6.4).

*6.15)* Players may also divide a stack of units in order to fire at different enemy-occupied hexes. Anglo-Egyptian infantry units having brigade integrity, however, do not receive their +1 direct fire modifier unless they all fire at the same enemy-occupied hex (see 5.54).

*6.16)* When halving fire combat strength, always round down each individual unit. For example, an Egyptian brigade of four battalions, each having a printed strength of 9 fire factors, will fire a total of 16 factors when halved. However, a unit's firing strength is never reduced below one by halving.

== How To Have Fire Combat

*6.21)* When combat units wish to fire at enemy units, first check the Line of Sight Table to be sure the firing unit can see the target hex (exception: howitzer fire, see 6.64).

*6.22)* Next consult the Range Effects Table to see if the firing unit's fire combat factor is tripled, doubled, normal, halved, or if the target hex is out of range. Add up the total number of fire combat factors firing at the enemy-occupied hex.

*6.23)* Next check the Terrain Effects Chart to see if the enemy-occupied hex fired upon contains any terrain which gives the enemy units in that hex a defensive benefit. If so, apply this negative modifier to the roll of the ten-sided die and cross-index your net die roll on the Combat Results Table with the number of combat factors firing.

*6.24)* All Anglo-Egyptian direct fire attacks receive a +1 modifier to their die roll as an accuracy bonus. In addition, any stack of Anglo-Egyptian infantry having brigade integrity (see 5.54) receives a +1 modifier to their die roll if all four fire at the same enemy-occupied hex. These modifiers are cumulative.

#align(center, image("Boardgame - Remember_Gordon/Manual/Elements/CombatResultsTable.jpg", width: 80%))
...
*6.41) Direct Fire Subphase (Dervish and Anglo-Egyptian players):*
...
*6.42) Maxim Second Fire and Howitzer Fire Subphase (Anglo-Egyptian player only):*
...
*6.52) Anglo-Egyptian "Friendlies" Brigade:*
...
*6.53) Anglo-Egyptian Royal Engineers (Royal Eng. 5-3-8):*
...
== Special Artillery Capabilities

*6.61)* Only artillery may fire at gunboats. A result of 3 or more on the combat results table is required to sink a gunboat. Any other result is a miss.

*6.62)* Only artillery may fire at forts. A result of 2 or more on the combat results table is required to eliminate a fort. Any other result is a miss. If the fort contains any enemy units at the instant it is destroyed, one unit is eliminated with the fort.

*6.63)* Only artillery may fire to breach a wall hexside of Khartoum or the walled city of Omdurman. A result of 2 or more on the combat results table is required to breach a wall. Any other result is a miss. The effect of the breach is to negate the wall hexside for line of sight purposes. Place a "BREACH" marker in an adjacent hex so that the arrow points to the breached hexside. If any enemy units are adjacent to the wall hexside at the instant it is breached, one enemy unit is eliminated.
...
*6.81)* During Offensive Fire phase, the moving player may fire with all of his units capable of firing, up to their maximum range, within the limitations imposed by the rules of combat.
...
*7.1)* The melee strength of all units is printed on the counter. Note that gunboats have no melee strength. Gunboats may neither melee attack nor be melee attacked.

*7.2)* Melee simulates the hand-to-hand fighting of the period. Units may melee attack adjacent enemy units only. Units may not melee attack across a wall hexside, but may melee attack through a gate or breach hexside.

*7.3)* Melee combat is considered simultaneous, so that units eliminated by melee attacks still get a melee combat die roll.

*7.4)* Only infantry, cavalry, camel units, and Dervish leaders may melee attack. All units (except gunboats — see 7.1) may melee defend.

*7.5)* Cavalry and camel units may retreat two hexes from an infantry melee attack. Note, however, that only one retreat per unit per turn is permitted. Thus, if their retreat places them adjacent to enemy units whose melee attacks have not yet been resolved, those enemy units may elect to attack the retreating unit(s).

*7.6)* If a melee attack eliminates all of the defenders in an adjacent hex, the Dervish player MUST advance into the vacated hex. To be eligible to advance, the Dervish units must have been adjacent to the vacated hex and participated in the melee attack that eliminated the defenders. All surviving eligible Dervish units MUST advance, up to the stacking limit. The Anglo-Egyptian player may advance if desired. Note that only attacking units may advance.

*7.7)* To resolve melee, both the attacker and the defender roll on the Combat Results Table and apply the applicable melee modifier to their die roll. The Dervish player receives a +2 melee modifier, the Anglo-Egyptian player receives a +1 melee modifier. No terrain modifiers are applied to melee combat (Exception: Zariba hexsides in the historical scenario and the campaign game, if constructed — see 9.23). Melee losses must be taken from meleeing units first!

#pagebreak()

= Night Game Turns

*8.1)* The effects of night game turns are: a) all Anglo-Egyptian movement allowances are halved (round down), b) there is no Anglo-Egyptian howitzer fire, and c) all fire ranges are halved for both sides (round down, but range 1 stays range 1). Range effects on fire combat are the same as during day game turns. For example, an Anglo-Egyptian infantry unit firing at night will be doubled at range 1, normal at range 2, and may not fire at range 3 or greater.
...
*9.111)* Dervish player sets up first, moves second.
...
*9.113)* The Anglo-Egyptian player moves first. There are no Anglo-Egyptian units on the mapsheet at start. The GORDON unit is not used in this scenario.
...
#align(center, table(
  ...
  [*Victory Level*], [*Dervish superiority*], [*Anglo-Egyptian superiority*],
...
#align(center, image("Boardgame - Remember_Gordon/Manual/Elements/CampaignGameOrdnerOfAppearance.jpg", width: 75%))
...
*9.211)* The Anglo-Egyptian player sets up first, and moves second:
...
*9.212)* The Dervish player sets up second, and moves first.
...
*9.233)* Units may only enter and/or leave the Zariba via the two end hexsides that connect to the Nile River, paying +2 movement points to cross (Exception: advance after combat across an entrenched hexside).
...
#align(center, table(
  ...
  stroke: 0.5pt,
  align: center + horizon,
  [*Victory Level*], [*Anglo-Egyptian* \ (Dervish elim.)], [*Dervish* \ (A-E elim.)],
  [5 — DECISIVE], [100+], [30+],
...
== Bonus Game: FALL OF KHARTOUM

#align(center, image("Boardgame - Remember_Gordon/Manual/Elements/Manual_10.jpg", width: 80%))

*9.31)* Only the small FALL OF KHARTOUM scenario map is used for this game.

=== Set Up

*9.321)* The British player sets up first, moves second:
...
*9.322)* Dervish player moves first: enters turn one through any hexes on the south or east edge of the map.
...
*9.341)* Turn 1 is always a night turn (see 8.1).

*9.342)* All hexes are playable, including hexes showing up half or less.

*9.343)* Both players must use the Dervish Range Effects Table.

*9.344)* The Dervish player controls the "North Fort" and may fire its guns.

*9.345)* The British gunboats may move from the White Nile to the Blue Nile and vice-versa at an off-board movement cost of six "upstream" movement points.

*9.346)* The GORDON leader unit starts in the palace and may not move during the scenario. He may only be eliminated by a Dervish unit passing through or occupying the palace hex (as normal movement or as advance after combat).
...
*10.11)* Prior to the commencement of play the Dervish player secretly records two Nile River hexes to be mined (the mines may not both be placed in the same hex). These hexes must be south of the E–W hexrow in which the Khor Shambat empties into the Nile.

*10.12)* When a British gunboat enters a mined hex, the Dervish player must order it to stop as it has struck a mine. The Dervish player then resolves the effect of the mine's blast by rolling the ten-sided die:
...
*10.13)* After both mines have been rolled for, no more are available.

*10.14)* The Dervish player's gunboats may pass through the mined hexes with no ill effect (he knows where the mines are).
...
*10.21)* Prior to the commencement of play the Dervish player secretly records a line of river hexes (not exceeding four hexes long) across which the chain is strung. The hexes must be south of the E–W hexrow in which the Khor Shambat empties into the Nile.

*10.22)* When a British gunboat enters a "chained" river hex it must stop and may move no further that turn.
...
#align(center, image("Boardgame - Remember_Gordon/Manual/Elements/Manual_04.jpg", width: 80%))
...
#align(center, image("Boardgame - Remember_Gordon/Manual/Elements/Manual_06.jpg", width: 80%))
...
#align(center, image("Boardgame - Remember_Gordon/Manual/Elements/Manual_08.jpg", width: 80%))
